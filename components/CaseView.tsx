...
import FingerprintAnalysisGame from "./puzzles/FingerprintAnalysisGame";
import ChemicalAnalysisGame from "./puzzles/ChemicalAnalysisGame";
...
const CaseView: React.FC<CaseViewProps> = ({ caseData, onExit }) => {
  ...
  const [eliminatedSuspects, setEliminatedSuspects] = useState<Set<string>>(
    new Set(),
  );
    ...
    if (caseData.puzzleUnlockStrategy === "all") {
      ...
      caseData.puzzles.forEach((p, index) => {
        initialStatuses[p.id] =
          index === 0 ? PuzzleStatus.UNLOCKED : PuzzleStatus.LOCKED;
      });
    }
  ...
  const handlePuzzleComplete = (puzzleId: string, resultText: string) => {
    ...
    const newResult: PuzzleResult = {
      ...
      isMatch:
        ...
        resultText.toLowerCase().includes("analysis") ||
        resultText.toLowerCase().includes("positive") ||
        resultText.toLowerCase().includes("confirmed"),
    };
      ...
      if (nextPuzzleIndex < caseData.puzzles.length) {
        ...
        if (statuses[nextPuzzleId] === PuzzleStatus.LOCKED) {
          setTimeout(() => {
            setStatuses((prev) => ({
              ...prev,
              [nextPuzzleId]: PuzzleStatus.UNLOCKED,
            }));
          }, 300);
        }
      }
  ...
  const handlePlayPuzzle = (puzzleId: string) => {
    ...
    console.log("puzzleToPlay", puzzleToPlay, puzzleId);
    if (puzzleToPlay) {
      if (puzzleToPlay.puzzleType === "PLACEHOLDER") {
        ...
        setStatuses((prev) => ({
          ...prev,
          [puzzleId]: PuzzleStatus.IN_PROGRESS,
        }));
        console.log("puzzleToPlay", puzzleToPlay);
  ...
  const renderMinigame = () => {
    ...
    switch (activePuzzle.puzzleType) {
      ...
      case "CCTV_RECONSTRUCTION":
        return (
          <CctvReconstructionGame
            puzzle={activePuzzle}
            onComplete={handlePuzzleComplete}
          />
        );
      case "CHEMICAL_ANALYSIS":
        return (
          <ChemicalAnalysisGame
            puzzle={activePuzzle}
            onComplete={handlePuzzleComplete}
          />
        );
  ...
  const TabButton: React.FC<{
    ...
    <button
      ...
      className={`flex-1 py-2 px-4 font-teko text-2xl tracking-wider uppercase transition-colors ${
        activeTab === tab
    ...
    <div className="space-y-4 animate-fade-in-fast">
      {suspects
        ...
        .map((suspect) => (
          <div
            key={suspect.name}
            className={`flex flex-col sm:flex-row items-start p-4 bg-slate-800 border border-slate-700 rounded-lg gap-4 transition-all duration-500 ${
              animating.has(suspect.name) ? "animate-shrink-out" : ""
            }`}
          >
            <img
              src={suspect.img}
              alt={suspect.name}
              className="w-full sm:w-32 h-48 sm:h-auto object-cover rounded-md border-2 border-slate-600 shrink-0"
            />
            <div className="flex-grow">
              <h3 className="font-teko text-3xl text-slate-100">
                {suspect.name}
              </h3>
              <p className="text-sm text-slate-400 mb-3 italic">
                "{suspect.connectionToScene || suspect.description}"
              </p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2 text-sm">
                {suspect.height && (
                  <div>
                    ...
                    <span className="text-slate-400">{suspect.height}</span>
                  </div>
                )}
                {suspect.weight && (
                  <div>
                    <span className="font-bold text-slate-300">Weight:</span>{" "}
                    <span className="text-slate-400">{suspect.weight}</span>
                  </div>
                )}
                {suspect.shoeSize && (
                  <div>
                    <span className="font-bold text-slate-300">Shoe Size:</span>{" "}
                    <span className="text-slate-400">{suspect.shoeSize}</span>
                  </div>
                )}
                {suspect.bloodType && (
                  <div>
                    ...
                    <span className="text-slate-400">{suspect.bloodType}</span>
                  </div>
                )}
                {suspect.toneOfVoice && (
                  <div>
                    <span className="font-bold text-slate-300">Voice:</span>{" "}
                    <span className="text-slate-400">
                      {suspect.toneOfVoice}
                    </span>
                  </div>
                )}
                {suspect.gunOwner !== undefined && (
                  <div>
                    <span className="font-bold text-slate-300">Gun Owner:</span>{" "}
                    <span className="text-slate-400">
                      {suspect.gunOwner ? "Yes" : "No"}
                    </span>
                  </div>
                )}
                {suspect.financialStanding && (
                  <div>
                    <span className="font-bold text-slate-300">Finances:</span>{" "}
                    <span className="text-slate-400">
                      {suspect.financialStanding}
                    </span>
                  </div>
  ...
  return (
    <div className="p-4 relative">
      ...
      <header className="flex items-center justify-between mb-4">
        <h1 className="font-teko text-4xl font-bold text-blue-400 uppercase tracking-wide">
          {caseData.title}
        </h1>
        <button
          onClick={onExit}
      ...
      {allPuzzlesCompleted ? (
        ...
        <>
          <div className="mb-4 p-4 bg-slate-900/50 border border-slate-700/50 rounded-lg">
            <h2 className="font-teko text-2xl text-slate-300 border-b border-slate-700 pb-1 mb-2">
              Case Brief
            </h2>
