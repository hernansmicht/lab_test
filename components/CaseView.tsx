    ...
    <div className="space-y-4 animate-fade-in-fast">
      {suspects
        ...
        .map((suspect) => (
          <div
            key={suspect.name}
            className={`flex items-start p-4 bg-slate-800 border border-slate-700 rounded-lg gap-6 transition-all duration-500 ${
              animating.has(suspect.name) ? "animate-shrink-out" : ""
            }`}
          >
            <img
              src={getAssetPath(suspect.img)}
              alt={suspect.name}
              className="w-48 h-64 object-cover rounded-lg border-2 border-slate-600 shrink-0"
            />
            <div className="flex-grow">
              <div className="mb-4">
                <h3 className="font-teko text-4xl text-slate-100">
                  {suspect.name}
                </h3>
